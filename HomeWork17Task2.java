...
class Basket {
    ...
    public void purchaseProducts(final List<Product> productsCatalogue) {
        ...
        LocalDateTime dateTransaction = LocalDateTime.now(zone);
        ZonedDateTime dateTransactionZoned = ZonedDateTime.of(dateTransaction, zone);

        for (Product currentProduct : productsCatalogue) {
            productsPurchased.put(currentProduct, dateTransactionZoned);
        }
    }
